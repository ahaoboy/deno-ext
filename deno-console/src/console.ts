...
import { core, internals, primordials } from "@deno-ext/core";
    ...
    if (noIterator) {
      ...
      if (constructor === "Object") {
        ...
        if (typedArray === undefined) {
          ...
          return prefix +
            `{ byteLength: ${formatNumber(ctx.stylize, TypedArrayPrototypeGetByteLength(value))
            } }`;
        }
        braces[0] = `${prefix}{`;
...
function formatMap(value, ctx, _ignored, recurseTimes) {
  ...
  for (let i = 0; i < len; i++) {
    ArrayPrototypePush(
      output,
      `${formatValue(ctx, values[i][0], recurseTimes)} => ${formatValue(ctx, values[i][1], recurseTimes)
      }`,
    );
  }
  if (remaining > 0) {
