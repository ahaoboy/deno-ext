...
import { core, internals } from "@deno-ext/core";
import { op_node_ipc_read, op_node_ipc_write } from "@deno-ext/core/ops";
import {
  ArrayIsArray,
...
} from "@deno-ext/node/internal/primordials.mjs";

import { assert } from "@deno-ext/node/_util/asserts";
import { EventEmitter } from "bufferevents";
import { os } from "@deno-ext/node/internal_binding/constants.ts";
import { notImplemented, warnNotImplemented } from "@deno-ext/node/_utils";
import { Readable, Stream, Writable } from "bufferstream";
import { isWindows } from "@deno-ext/node/_util/os";
import { nextTick } from "@deno-ext/node/_next_tick.ts";
import {
  AbortError,
  ERR_INVALID_ARG_TYPE,
  ERR_INVALID_ARG_VALUE,
  ERR_UNKNOWN_SIGNAL,
} from "@deno-ext/node/internal/errors";
import { Buffer } from "bufferbuffer";
import { errnoException } from "@deno-ext/node/internal/errors";
import { ErrnoException } from "@deno-ext/node/_global.d.ts";
import { codeMap } from "@deno-ext/node/internal_binding/uv";
import {
  isInt32,
  validateBoolean,
  validateObject,
  validateString,
} from "@deno-ext/node/internal/validators.mjs";
import { kEmptyObject } from "@deno-ext/node/internal/util.mjs";
import { getValidatedPath } from "@deno-ext/node/internal/fs/utils.mjs";
import process from "bufferprocess";
