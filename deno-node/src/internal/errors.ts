...
import { primordials } from "@deno-ext/core";
const { JSONStringify } = primordials;
import { format, inspect } from "@deno-ext/node/internal/util/inspect";
import { codes } from "@deno-ext/node/internal/error_codes";
import {
  codeMap,
  errorMap,
  mapSysErrnoToUvErrno,
} from "@deno-ext/node/internal_binding/uv";
import { assert } from "@deno-ext/node/_util/asserts";
import { isWindows } from "@deno-ext/node/_util/os";
import { os as osConstants } from "@deno-ext/node/internal_binding/constants.ts";
import { hideStackFrames } from "@deno-ext/node/internal/hide_stack_frames";
import { getSystemErrorName } from "@deno-ext/node/_utils";
...
export class ERR_INVALID_PACKAGE_CONFIG extends NodeError {
  constructor(path: string, base?: string, message?: string) {
    const msg = `Invalid package config ${path}${base ? ` while importing ${base}` : ""
      }${message ? `. ${message}` : ""}`;
  ...
  constructor(
    ...
    if (key === ".") {
      ...
      msg = `Invalid "exports" main target ${JSON.stringify(target)} defined ` +
        `in the package config ${displayJoin(pkgPath, "package.json")}${base ? ` imported from ${base}` : ""
        }${relError ? '; targets must start with "./"' : ""}`;
    } else {
      msg = `Invalid "${isImport ? "imports" : "exports"}" target ${JSON.stringify(
        target,
      )
        } defined for '${key}' in the package config ${displayJoin(pkgPath, "package.json")
        }${base ? ` imported from ${base}` : ""}${relError ? '; targets must start with "./"' : ""
        }`;
    }
...
export class ERR_PACKAGE_IMPORT_NOT_DEFINED extends NodeTypeError {
  constructor(
    ...
    const msg = `Package import specifier "${specifier}" is not defined${packagePath
...
export class ERR_PACKAGE_PATH_NOT_EXPORTED extends NodeError {
  constructor(subpath: string, pkgPath: string, basePath?: string) {
    let msg: string;
    if (subpath === ".") {
      msg = `No "exports" main defined in ${displayJoin(pkgPath, "package.json")
        }${basePath ? ` imported from ${basePath}` : ""}`;
    } else {
      msg = `Package subpath '${subpath}' is not defined by "exports" in ${displayJoin(pkgPath, "package.json")
        }${basePath ? ` imported from ${basePath}` : ""}`;
    }
